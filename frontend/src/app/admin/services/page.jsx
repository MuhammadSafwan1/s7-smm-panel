              ...
              {filtered.map(svc => (
                <div key={svc.service} onClick={() => onSelect(svc)}
                  ...
                  <div className="text-right flex-shrink-0">
                    <p className="text-sm font-bold text-primary-600 dark:text-primary-400">${parseFloat(svc.rate||0).toFixed(4)}</p>
...
export default function ServicesPage() {
  ...
  const openEdit = (svc) => {
    ...
    setForm({ serviceId: svc.serviceId||'', name: svc.name, platformId: svc.platformId, categoryId: svc.categoryId, providerId: svc.providerId||'', providerServiceId: svc.providerServiceId||'', price: svc.price, minQuantity: svc.minQuantity, maxQuantity: svc.maxQuantity, avgTime: svc.avgTime||'1-6 Hours', description: svc.description||'', isActive: svc.isActive, isFeatured: svc.isFeatured||false, isPopular: svc.isPopular||false, refillSupported: svc.refillSupported||false, cancelSupported: svc.cancelSupported||false, refundSupported: svc.refundSupported||false });
    setShowModal(true);
  };
                          ...
                          : <div className="divide-y divide-dark-100 dark:divide-dark-800">
                            {svcs.map((svc, idx) => (
                              <div key={svc.id} className={`flex items-center gap-3 px-6 py-3 hover:bg-dark-50 dark:hover:bg-dark-800/30 transition-colors group ${!svc.isActive ? 'opacity-50' : ''}`}>
                                <span className="text-xs font-mono bg-dark-100 dark:bg-dark-800 text-dark-500 px-2 py-1 rounded-lg flex-shrink-0 min-w-[48px] text-center">#{idx+1}</span>
                                <div className="flex-1 min-w-0">
                                  ...
                                  <div className="flex items-center gap-2 mt-0.5 flex-wrap text-xs text-dark-400">
                                    ...
                                    {svc.serviceId && <span className="text-primary-400 font-mono font-bold">#{svc.serviceId}</span>}
                                  </div>
                                </div>
                                <span className="text-sm font-bold text-primary-600 dark:text-primary-400 flex-shrink-0">${parseFloat(svc.price||0).toFixed(2)}/1k</span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="bg-white dark:bg-dark-900 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSave} className="px-6 py-5 space-y-5">
              ...
              {/* Service ID */}
              <div>
                <label className={labelCls}>Service ID <span className="text-xs font-normal text-dark-400 ml-1">(shown to users — e.g. 1, 2, 100)</span></label>
                <input type="text" placeholder="e.g., 1001"
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className={labelCls}>Price per 1000 ($) *</label>
                  <input type="number" step="0.0001" required placeholder="0.50" value={form.price} onChange={e => setForm({ ...form, price: e.target.value })} className={inputCls} />
                </div>
                <div>
              ...
              <div>
                ...
                <textarea rows="2" placeholder="Service details..." value={form.description} onChange={e => setForm({ ...form, description: e.target.value })} className={inputCls + ' resize-none'} />
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-3 gap-3">
                  {[['isActive','✓ Active'],['isFeatured','★ Featured'],['isPopular','🔥 Popular'],['refillSupported','↩ Refill'],['cancelSupported','✕ Cancel'],['refundSupported','$ Refund']].map(([key, label]) => (
        ...
        <ProviderBrowseModal
          ...
          onSelect={svc => {
            setForm(prev => ({
              ...
              price: parseFloat(svc.rate || 0).toFixed(4),
              minQuantity: parseInt(svc.min || 100),
              maxQuantity: parseInt(svc.max || 100000),
              refillSupported: svc.refill === true || svc.refill === 'true',
              cancelSupported: svc.cancel === true || svc.cancel === 'true',
            }));
            setShowBrowse(false);
            toast.success(`"${svc.name}" imported — set platform/category then save`);
          }}
        />
      )}
    </div>
